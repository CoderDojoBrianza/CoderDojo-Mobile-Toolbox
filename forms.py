...
class CheckInOutForm(forms.Form):
    participant_id = forms.CharField(label=_('Participant_Id'), max_length=200,required=False)
    ticket_id = forms.CharField(label=_('Ticket Id'), max_length=200,required=False)
    participant = ParticipantModelChoiceField(label=_('Participant'),queryset=None,required=False)  # Here we use a list of participants from the db (we assume it's small!)
    ...
    check_in_out=forms.ChoiceField(label=_('Check_in_or_cancel'),choices=CHECK_CHOICES,widget=forms.RadioSelect,initial=CHECK_IN)
